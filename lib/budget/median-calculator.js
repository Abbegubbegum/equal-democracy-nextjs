	...
	for (const category of session.categories) {
		...
		for (const subcategory of category.subcategories || []) {
			...
			subcategories.push({
				...
				percentageOfCategory: medianAmount > 0
					? (medianSubAmount / medianAmount) * 100
					: 0,
			});
		}
...
export function validateBudgetVote(vote, session) {
	...
	for (const allocation of vote.allocations) {
		const category = session.categories.find((c) => c.id === allocation.categoryId);

		if (!category) {
			errors.push(`Category ${allocation.categoryId} not found`);
			continue;
		}
		...
		if (allocation.amount < category.minAmount) {
			errors.push(
				`${category.name}: Amount ${allocation.amount} is below minimum ${category.minAmount}`
			);
		}
		...
		for (const subAllocation of allocation.subcategories || []) {
			...
			if (!subcategory) {
				errors.push(`Subcategory ${subAllocation.subcategoryId} not found`);
				continue;
			}

			if (subAllocation.amount < subcategory.minAmount) {
				errors.push(
					`${category.name} - ${subcategory.name}: Amount ${subAllocation.amount} is below minimum ${subcategory.minAmount}`
				);
			}
		}
	}
