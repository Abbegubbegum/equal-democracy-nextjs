...
export async function sendLoginCode(email, code) {
  ...
  `;
	await resend.emails.send({ to: email, from: FROM_ADDRESS, subject, text, html });
}

export async function sendSessionResultsEmail(email, sessionName, topProposals) {
	const subject = `Tack för ditt deltagande i ${sessionName}`;
	...
	if (topProposals.length > 0) {
		proposalsList = topProposals
			.map(
				(p, i) =>
					`${i + 1}. ${p.title} (${p.yesVotes} ja-röster, ${p.noVotes} nej-röster)`
			)
			.join("\n");
	} else {
...
`;
    ...
    <div style="font-family:system-ui,-apple-system,Segoe UI,Roboto,sans-serif;line-height:1.5;color:#1e293b">
      ...
      <p><strong>Omgång:</strong> ${sessionName}</p>
