...
- initialArguments kan vara tom array om inga argument finns`;
	...
	try {
		...
		console.log("[AgendaExtractor] Claude response preview:", responseText.substring(0, 300));
		...
		if (!jsonMatch) {
			console.error("[AgendaExtractor] Full response:", responseText);
		...
		console.log("[AgendaExtractor] Extracted data summary:", {
		...
		console.error("[AgendaExtractor] Error calling Claude AI:", error);
...
export async function extractAgendaFromURL(url, meetingType = "Kommunfullmäktige") {
	try {
		console.log(`[AgendaExtractor] Fetching PDF from URL: ${url}`);
		...
		console.log(`[AgendaExtractor] PDF downloaded, size: ${pdfBuffer.length} bytes`);

		return await extractAgendaFromPDF(pdfBuffer, meetingType);
	} catch (error) {
		console.error("[AgendaExtractor] Error fetching/extracting from URL:", error);
